...
|---|---|---|
| [Criar um pod racer voxel com um prompt](#case-1) | Use uma ideia curta para prototipar uma corrida voxel e planejar a próxima versão | Demo |
| [Comparar Frogger com o mesmo prompt](#case-2) | Mantenha o prompt constante para inspecionar diferenças entre modelos | Evaluation |
| [Gerar Frogger e sua gravação](#case-3) | Teste em uma execução o jogo e o fluxo de gravação | Demo |
| [Prototipar um porta-aviões em Three.js](#case-4) | Use uma cena concreta para testar um protótipo 3D interativo | Demo |
| [Criar um jogo inspirado em Paper Mario com ferramentas de agente](#case-9) | Combine Kimi K3 com um harness de agente e ferramentas de assets para produzir elementos de jogo 2D e 3D | Demo |
| [Gerar um jogo de tiro em primeira pessoa no metrô](#case-11) | Use um cenário concreto de metrô para examinar o resultado de um FPS gerado | Demo |
| [Modelar um motor V8 com Blender MCP](#case-19) | Use Blender MCP e um único pedido para gerar um modelo mecânico 3D detalhado | Integration |
| [Criar uma arena de batalha jogável a partir de uma referência](#case-23) | Use uma única referência para testar a geração em uma execução de uma arena jogável completa | Demo |
| [Criar três jogos retrô autônomos como arquivos HTML](#case-24) | Exija gráficos, inimigos, regras e jogo autônomo em arquivos HTML independentes | Benchmark |
| [Criar um jogo de esconde-esconde com camaleão em uma execução](#case-27) | Gere um jogo em arquivo único com combinação de cores, áreas procedurais, som e pontuação por várias rodadas | Benchmark |
| [Criar um jogo 2.5D no estilo Paper Mario com uma cadeia de agentes](#case-36) | Use Kimi K3 com Grok Build ou Claude Code e Spriterrific para montar um fluxo de jogo 2.5D | Tutorial |
| [Criar um RPG wuxia 3D no navegador](#case-43) | Combine combate corpo a corpo, missões, inventário, clima, interiores, trabalho de ambiente no Blender e assets adaptados | Demo |
| [Criar um jogo multiplayer de navegador no estilo Minecraft](#case-44) | Use uma execução delimitada por tempo e custo para produzir um jogo multiplayer online jogável no navegador | Demo |
| [Recriar um jogo cooperativo de tela dividida no navegador](#case-48) | Use um pedido para gerar cooperação em tela dividida no navegador e interação com o ambiente em tempo real | Demo |
| [Gerar um jogo jogável com o modo de design da Command Code](#case-49) | Use o comando de design da Command Code para criar um jogo em uma execução e registre se o resultado é jogável | Demo |
| [Montar um RPG wuxia coeso no navegador](#case-51) | Integre travessia, combate, missões, inventário, clima, exploração e trabalho de ambiente 3D em um jogo | Demo |
| [Criar um crossover jogável de Hollow Knight](#case-54) | Use assets existentes do jogo para criar uma batalha jogável entre o Cavaleiro e Kimi | Demo |
| [Criar em uma execução um jogo 3D no estilo Fall Guys para navegador](#case-60) | Use um pedido em uma execução para gerar um jogo 3D de obstáculos jogável e disponibilize o projeto para inspeção | Demo |
| [Criar e testar automaticamente um FPS apocalíptico em Lisboa](#case-61) | Use uma execução de esforço máximo com um prompt que teste, capture telas e itere antes de entregar um FPS jogável | Demo |
| [Gerar um jogo no estilo Animal Crossing a partir de um pedido simples](#case-63) | Use um briefing mínimo de jogo para examinar jogabilidade, ciclo de jogo e efeitos de paralaxe | Demo |
| [Gerar um jogo no estilo Mario a partir de uma frase](#case-65) | Use um pedido mínimo em uma execução para examinar jogabilidade, design de fases, pixel art e paralaxe | Demo |
| [Criar um jogo de tiro em primeira pessoa com zumbis funcional](#case-67) | Use um alvo concreto de jogo de tiro com zumbis para examinar um artefato FPS completo e jogável | Demo |
...
| Case | What it shows | Type |
|---|---|---|
| [Criar um frontend de motion graphics interativo](#case-5) | Crie gráficos que continuem interativos quando pausados | Demo |
| [Produzir um anúncio animado sincronizado](#case-6) | Verifique a sincronia entre música, efeitos e movimento | Demo |
| [Criar motion design inteiramente em código](#case-14) | Teste se um fluxo de programação em uma execução consegue produzir motion design sem ferramentas auxiliares de geração | Demo |
| [Pesquisar uma pessoa e criar um site pessoal animado](#case-15) | Forneça um briefing amplo de site pessoal e examine pesquisa, planejamento, iteração e validação no navegador pelo modelo | Tutorial |
| [Gerar uma simulação de buraco negro](#case-17) | Use uma tarefa de visualização científica para examinar uma simulação gerada de buraco negro | Demo |
| [Testar modelagem complexa, partículas e shaders no frontend](#case-22) | Use um prompt público de frontend para examinar precisão de modelagem, efeitos de partículas e geração de shaders inline em uma passagem | Demo |
| [Gerar uma ferramenta de música procedural em uma tentativa](#case-26) | Teste a geração em uma execução de um gerador interativo de música procedural e compare o resultado visível com cautela | Demo |
| [Criar uma página de produto em Three.js a partir de duas imagens](#case-33) | Use duas imagens de referência e uma exigência explícita de Three.js para gerar uma apresentação de produto | Demo |
| [Inventar um cortador de pão de luxo e sua página de produto](#case-39) | Combine ideação de produto, vista explodida, demonstração e landing page em um artefato de design | Demo |
| [Gerar um GIF recursivo de pelicano com dez segundos](#case-45) | Use um briefing completo de animação em loop para examinar continuidade narrativa e recursão em um GIF | Demo |
| [Gerar um SVG lateral de um BMW M4 CS](#case-55) | Use um veículo e ponto de vista específicos para examinar a saída de ilustração vetorial | Demo |
| [Recriar Gargantua por feedback de capturas de tela](#case-58) | Use capturas de tela repetidas como observações para diagnosticar e refinar uma visualização científica | Tutorial |
| [Refinar uma visualização de buraco negro com 62 capturas de tela](#case-66) | Use um ciclo de feedback por capturas de tela para ler, diagnosticar e corrigir uma simulação visual ao longo de muitas iterações | Tutorial |
| [Criar um PDF de marketing sobre pós-treinamento](#case-68) | Use um produto nomeado e um formato de entrega para gerar um documento de marketing | Demo |
| [Criar uma interface de usuário a partir de um prompt](#case-70) | Use um único pedido para gerar e examinar um design completo de interface | Demo |
...
| Case | What it shows | Type |
|---|---|---|
| [Criar um MacBook virtual com macOS funcional](#case-18) | Combine renderização de hardware em Three.js com uma simulação interativa de sistema operacional no navegador | Demo |
| [Criar um compilador de GPU de DSL a PTX](#case-25) | Use uma tarefa completa de compilador que abranja DSL, passes, geração de PTX e um caminho de Tensor Core | Demo |
| [Criar um raytracer de buraco negro em tempo real no WebGL2](#case-32) | Teste a geração, com um prompt, de um raytracer geodésico nativo em WebGL2 dentro de um arquivo HTML | Benchmark |
| [Criar um emulador de Game Boy Advance com mGBA WASM](#case-46) | Integre um modelo 3D licenciado e um núcleo real de emulador e depois melhore recursivamente interface e jogabilidade | Integration |
| [Pesquisar vários temas em fontes de língua chinesa](#case-50) | Use tarefas de pesquisa longas para comparar profundidade e latência entre gerações de modelos | Evaluation |
| [Clonar o macOS no navegador com aplicativos funcionais](#case-56) | Crie uma simulação de sistema operacional no navegador que inclua aplicativos de música, navegador e e-mail | Demo |
| [Criar uma simulação de macOS com FaceTime funcional](#case-62) | Use uma tarefa de sistema operacional virtual para testar se uma interação de aplicativo gerada funciona | Demo |
| [Adicionar um comparador de efeitos de frontend com duas tarefas](#case-64) | Crie uma ferramenta que selecione duas tarefas concluídas, exiba-as lado a lado e sincronize visualizações e interações | Tutorial |
...
| Case | What it shows | Type |
|---|---|---|
| [Comparar design de frontend na tarefa de lava lamp do BridgeBench](#case-7) | Use a tarefa de lava lamp do BridgeBench como uma comparação delimitada de design de frontend, não como ranking universal | Benchmark |
| [Avaliar roteiros com voz editorial](#case-8) | Meça adequação à voz editorial, posição relativa e custo por roteiro dentro de um benchmark interno claramente identificado | Benchmark |
| [Comparar design, custo e dificuldade de jogos no estilo Flappy](#case-10) | Registre configurações de dificuldade, custo, design e recursos de jogabilidade ao comparar jogos gerados | Benchmark |
| [Comparar design de jogos com o mesmo prompt de design](#case-12) | Mantenha o prompt de design constante e examine ritmo, senso de design e sensação de jogabilidade separadamente | Benchmark |
| [Exigir revisão independente em auditorias estatísticas](#case-13) | Combine auditorias estatísticas geradas por modelo com revisão independente de especialista ou de outro modelo antes de confiar nos achados | Limit |
| [Avaliar uma execução de frontend lenta, mas forte](#case-16) | Registre o tempo de conclusão junto com a qualidade da saída ao testar uma tarefa de frontend | Evaluation |
| [Testar falhas de prenúncio em narrativas de mistério](#case-20) | Avalie se um mistério gerado equilibra pistas, obscuridade e prenúncio | Limit |
| [Comparar modelagem e animação da Millennium Falcon](#case-21) | Use pedidos de estilo e configurações de esforço equivalentes para comparar modelagem 3D, animação, tempo e custo | Benchmark |
| [Analisar uma coleção de dez projetos com Kimi K3](#case-28) | Use uma coletânea de projetos vinculada para descobrir artefatos concretos que devem ser verificados separadamente | Evaluation |
| [Comparar uma landing page avançada entre quatro modelos](#case-29) | Mantenha constante o pedido de landing page e examine profundidade de animação e conclusão nas saídas dos modelos | Evaluation |
| [Avaliar mecânicas e custos de jogos retrô](#case-30) | Compare jogabilidade, física, mecânicas, comportamento autônomo, uso de tokens e custo nas mesmas tarefas de jogos retrô | Benchmark |
| [Comparar geração de jogos com Fable 5](#case-31) | Use um exemplo lado a lado de jogos gerados como avaliação estreita, não como veredito amplo sobre os modelos | Evaluation |
| [Comparar tarefas complexas de frontend e desenvolvimento com Opus 4.8](#case-34) | Use várias tarefas complexas de programação para identificar vitórias e derrotas, sem declarar um modelo universalmente superior | Evaluation |
| [Analisar benchmarks e um teste de landing page](#case-35) | Combine contexto de benchmark com um teste concreto de geração de landing page, mantendo separados os dois tipos de evidência | Evaluation |
| [Avaliar raciocínio indutivo com tarefas de grafo para fórmula](#case-37) | Meça correção, comportamento em holdout e complexidade de fórmula em tarefas de indução de primeira ordem | Benchmark |
| [Analisar relatos sobre jogos, landing pages, 3D e contexto longo](#case-38) | Use uma coletânea de várias fontes para comparar artefatos concretos e observar limites de velocidade junto a alegações de custo | Evaluation |
| [Auditar um plano complexo e contestar suas soluções](#case-40) | Use um segundo modelo para identificar achados subestimados, soluções incorretas e conclusões que devem ser rejeitadas | Evaluation |
| [Comparar diagramas ASCII de aprendizado por reforço no estilo PPO](#case-41) | Mantenha constante um prompt de diagrama ASCII e compare como os modelos representam o ciclo de aprendizado por reforço | Evaluation |
| [Modelar no Blender enquanto se monitoram erros de capacidade](#case-42) | Avalie o progresso parcial no Blender junto com a confiabilidade do serviço, em vez de julgar apenas o artefato | Limit |
| [Comparar geração de Flappy Bird em uma arena](#case-47) | Use uma tarefa de arena para comparar dois resultados gerados de Flappy Bird, mantendo o julgamento específico à tarefa | Evaluation |
| [Resolver um problema de indução visual de Bongard com uma ferramenta](#case-52) | Teste se o uso de ferramentas ajuda a derivar a regra visual em uma tarefa de raciocínio de Bongard | Evaluation |
| [Comparar bom gosto em frontend e design 3D com GPT-5.6 Sol](#case-53) | Examine recursos, gosto visual, elegância e execução 3D em uma comparação delimitada de frontend | Evaluation |
| [Comparar geração de sites entre três modelos](#case-57) | Use saídas visíveis de sites para comparar Kimi K3, Fable 5 e GPT-5.6 Sol em um teste | Evaluation |
| [Comparar geração procedural de jogos 3D e custo](#case-59) | Mantenha um prompt constante entre modelos e examine roleta, caça-níquel e pinball gerados com o custo por execução | Benchmark |
| [Comparar detalhes e iluminação de uma cena 3D de arsenal](#case-69) | Examine densidade de objetos, iluminação e detalhes da cena em uma comparação delimitada entre Kimi K3 e Opus 4.8 | Evaluation |
