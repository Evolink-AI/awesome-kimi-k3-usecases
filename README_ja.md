...
129件のケースはすべて高信頼の公開ソースに基づきます。タイトルと作者は元投稿へリンクし、弱い候補、重複、証拠不足の候補は除外します

## 📊 概要

- 公開クリエイターと実務者による高信頼ケース 129 件
...
| Section | Cases |
|---|---|
| [インタラクティブゲームと3D](#games-3d) | 41 |
| [フロントエンドとモーションデザイン](#frontend-motion) | 32 |
| [コーディングと統合](#coding-integrations) | 18 |
| [評価と制約](#evaluation-limits) | 48 |
...
| [AA-BriefcaseでKimi K3を評価](#case-110) | 評価と制約 | リーダーボードの主張を鵜呑みにする前に、AA-Briefcaseの結果でKimi K3の知識労働系エージェント能力を判断する。 | Benchmark |
| [Geometry Dashクローンを比較](#case-111) | インタラクティブゲームと3D | 同じ要件のゲームbuildを比較し、Kimi K3がモバイル風の遊べるプロトタイプに十分か判断する。 | Evaluation |
| [Command Codeでレトロゲームを試す](#case-112) | インタラクティブゲームと3D | Command Codeの複数モデル設計テストで、Kimi K3のゲーム機能とモデル別コストを比較する。 | Evaluation |
| [フロントエンド順位の注意点を確認](#case-113) | 評価と制約 | Kimi K3のleaderboard勝利と、より広いcodingやcost主張を切り分けるための制約メモとして使う。 | Limit |
| [StackPerfの設計結果を比較](#case-114) | 評価と制約 | StackPerfでKimi K3とQwen 3.8の設計品質、速度、tool call挙動を比較する。 | Benchmark |
| [Three.jsのKimi対GLMを検証](#case-115) | フロントエンドとモーションデザイン | 同じ手続き型Three.js promptをKimi K3とGLM 5.2で走らせ、frontendと3D buildの挙動を比較する。 | Benchmark |
| [Space Discovererを評価](#case-116) | インタラクティブゲームと3D | Space Discovererミニベンチマークで、汎用的な3Dブラウザゲーム要件に対するモデル出力を比較する。 | Benchmark |
| [実道路のドライブゲームを作る](#case-117) | インタラクティブゲームと3D | Kimi K3をBlenderとGodotに組み合わせ、実在する近所の道路地図を使ったドライブゲームを試作する。 | Demo |
| [Monument Valley風promptの費用比較](#case-118) | 評価と制約 | 単一のMonument Valley風promptで、Kimi K3と他モデルの視覚品質、実行時間、費用を比較する。 | Benchmark |
| [claude-code-routerでKimiを使う](#case-119) | コーディングと統合 | 不要なproxy modeを避けながら、claude-code-router経由で既存のClaude Code workflowにKimi K3を組み込む。 | Integration |
| [Surge指数でKimi K3を評価](#case-120) | 評価と制約 | SurgeAIの分割結果を使い、Kimi K3の日常チャットでの強さと企業エージェントや科学での弱さを切り分けます。 | Benchmark |
| [参照付きランディングページを作成](#case-121) | フロントエンドとモーションデザイン | ランディングページ生成を試すときは、Kimi K3に強い視覚参照と反復的な指示を与えます。 | Demo |
| [縦型プラットフォームゲームを作成](#case-122) | インタラクティブゲームと3D | 短いゲーム概要で、Kimi K3が単一HTMLのブラウザ試作を返せるかを試します。 | Demo |
| [PDEソルバーの誤りを評価](#case-123) | 評価と制約 | PDEベンチマークの失敗を使い、Kimi K3周辺で記号・数値タスクに検証が必要な箇所を判断します。 | Benchmark |
| [ClinicalBenchのEHRケースを確認](#case-124) | 評価と制約 | ClinicalBenchのEHRケースを使い、Kimi K3が成功する場面と診断推論が失敗する場面を確認します。 | Benchmark |
| [オープンなエージェントハーネスを実行](#case-125) | コーディングと統合 | 共有エージェントハーネスを使い、ツール利用タスクでKimi K3と他の frontier モデルを比較します。 | Benchmark |
| [税務計算を評価](#case-126) | 評価と制約 | フロントエンドでの強さが税務計算にも移ると仮定せず、Kimi K3を個別にテストします。 | Limit |
| [生成Geometry Dashを検証](#case-127) | インタラクティブゲームと3D | Kimi K3のゲーム生成を完成品扱いする前に、ソルバーとブラウザ再生テストを組み合わせます。 | Benchmark |
| [3D粒子システムをシミュレート](#case-128) | フロントエンドとモーションデザイン | 粒子シミュレーションのプロンプトで、Kimi K3の動きの質をクローズドモデルと比較します。 | Evaluation |
| [Codex OAuthでKimiを実行](#case-129) | コーディングと統合 | Kimi K3のコーディング作業で手動APIキー設定を避けたい場合、Codex OAuth経路を使います。 | Integration |
| [複合 computer use ベンチマーク](#case-130) | 評価と制約 | 合成スコアだけでなく、複合的な computer use タスクで Kimi K3 を比較する。 | Benchmark |
| [深海解説パイプラインを作る](#case-131) | フロントエンドとモーションデザイン | 物語型の prompt を、シーン、テキスト、動き、解説を整理する視覚パイプラインに変換する。 | Demo |
| [六脚ロボットをシミュレートする](#case-132) | インタラクティブゲームと3D | Kimi K3 で脚の動きと見える挙動を持つ 3D ロボットシーンを試作する。 | Demo |
| [ランディングページをブラインドテストする](#case-133) | フロントエンドとモーションデザイン | モデル名を伏せた landing page 比較で、Kimi K3 の frontend 出力を評価する。 | Benchmark |
| [機械的な 3D ディテールを試作する](#case-134) | フロントエンドとモーションデザイン | Kimi K3 に、視覚ディテールとレンダリング制約を持つ小さな機械系 3D シーンを作らせる。 | Demo |
| [Expo モバイル UI を再現する](#case-135) | フロントエンドとモーションデザイン | Kimi K3 を mobile Expo UI で試し、構造、コンポーネント、視覚的な仕上げを確認する。 | Evaluation |
| [production backend をストレステストする](#case-136) | コーディングと統合 | より現実的な backend フローで Kimi K3 の統合、コード推論、限界を調べる。 | Evaluation |
| [opencodex 経由で Codex をルーティングする](#case-137) | コーディングと統合 | Kimi K3 を Codex/opencodex ループに接続し、具体的な agent routing を試す。 | Integration |
| [ゲーム調整ループを自動化する](#case-138) | インタラクティブゲームと3D | gameplay を録画し、分析し、パラメータを変え、再テストする harness を作る。 | Integration |
| [Web デザイン tutorial を追う](#case-139) | フロントエンドとモーションデザイン | 完整な tutorial を使い、Kimi K3 が layout、動き、視覚的な磨き込みをどう組むかを見る。 | Tutorial |
...
<a id="case-111"></a>
### Case 111: [Geometry Dashクローンを比較](https://x.com/Nekt_0/status/2079629004483465473) (by [@Nekt_0](https://x.com/Nekt_0))

**同じ要件のゲームbuildを比較し、Kimi K3がモバイル風の遊べるプロトタイプに十分か判断する。**

Nektは、Kimi K3が約30分と20万tokenで、ジャンプ、障害物、レベル、音楽、視覚効果を備えたGeometry Dash風クローンを作ったと述べ、添付動画でClaude Fable 5のbuildと比較している。
...
<a id="case-112"></a>
### Case 112: [Command Codeでレトロゲームを試す](https://x.com/naymur_dev/status/2079627963000398334) (by [@naymur_dev](https://x.com/naymur_dev))

**Command Codeの複数モデル設計テストで、Kimi K3のゲーム機能とモデル別コストを比較する。**

NaymurはCommand CodeでFable 5、GPT-5.6 Sol、Grok 4.5、Kimi K3に同じ/design promptを実行したと報告。Kimi K3はゲームプレイ、レベルアップ、弾、ボス戦を持つASCIIレトロゲームを生成し、費用は$0.15だったという。
...
<a id="case-116"></a>
### Case 116: [Space Discovererを評価](https://x.com/fabriciocarraro/status/2079548607393382528) (by [@fabriciocarraro](https://x.com/fabriciocarraro))

**Space Discovererミニベンチマークで、汎用的な3Dブラウザゲーム要件に対するモデル出力を比較する。**

Fabricio CarraroはClaude Fable 5、GPT-5.6 Sol、Kimi K3を最大reasoningで比較したミニベンチマークを報告。課題はSpace Discovererという3Dブラウザゲームで、3モデルすべてがWebGL 2上のThree.jsに収束したという。
...
<a id="case-117"></a>
### Case 117: [実道路のドライブゲームを作る](https://x.com/bijanbowen/status/2079526003643179102) (by [@bijanbowen](https://x.com/bijanbowen))

**Kimi K3をBlenderとGodotに組み合わせ、実在する近所の道路地図を使ったドライブゲームを試作する。**

Bijan Bowenは、Kimi K3がBlenderとGodotを使い、作者の近くの実際の道路区間をマップにしたドライブゲームを作っている様子を示している。添付動画がbuild状態の公開証拠である。
...
<a id="case-122"></a>
### Case 122: [縦型プラットフォームゲームを作成](https://x.com/diegocabezas01/status/2079946676270219488) (by [@diegocabezas01](https://x.com/diegocabezas01))

**短いゲーム概要で、Kimi K3が単一HTMLのブラウザ試作を返せるかを試します。**

Diego Cabezasは、足場が動く中でプレイヤーが上へ跳び続ける縦型プラットフォームゲームのKimi K3 Max実行を示しています。投稿は一回の実行で単一HTMLになったと記録しています。
...
<a id="case-127"></a>
### Case 127: [生成Geometry Dashを検証](https://x.com/Kirratr/status/2079902410042909108) (by [@Kirratr](https://x.com/Kirratr))

**Kimi K3のゲーム生成を完成品扱いする前に、ソルバーとブラウザ再生テストを組み合わせます。**

Kirratrは、同じGeometry Dash風ブリーフでKimi K3が36分53秒かかったと報告しています。ビームサーチソルバーが64ジャンプの勝利経路を見つけ、ブラウザで再生し、コンソールエラーなしだったとされています。
...
<a id="case-132"></a>
### Case 132: [六脚ロボットをシミュレートする](https://x.com/DilumSanjaya/status/2080335531108716750) (by [@DilumSanjaya](https://x.com/DilumSanjaya))

**Kimi K3 で脚の動きと見える挙動を持つ 3D ロボットシーンを試作する。**

出典: DilumSanjaya は Kimi K3 で作成した六脚ロボットのシミュレーションを共有している。物理ロボットの検証ではなく、インタラクティブ 3D デモとして保存する。
...
<a id="case-138"></a>
### Case 138: [ゲーム調整ループを自動化する](https://x.com/Oluwaphilemon1/status/2080187637806043200) (by [@Oluwaphilemon1](https://x.com/Oluwaphilemon1))

**gameplay を録画し、分析し、パラメータを変え、再テストする harness を作る。**

出典: Oluwaphilemon1 は Three.js プロジェクトでの Kimi K3 利用と、ゲーム内調整ループを説明している。手動 playtesting を減らす流れだが、公開投稿に基づく範囲に留める。
...
<a id="case-115"></a>
### Case 115: [Three.jsのKimi対GLMを検証](https://x.com/thehypedotnews/status/2079553731218325840) (by [@thehypedotnews](https://x.com/thehypedotnews))

**同じ手続き型Three.js promptをKimi K3とGLM 5.2で走らせ、frontendと3D buildの挙動を比較する。**

The HypeはKimi K3とGLM 5.2に、CRT上のcanvas texture付き80年代リビング、駆動系kinematics付きロードバイク、ガラス水槽という3つの単一HTML/Three.js promptを実行したという。
...
<a id="case-121"></a>
### Case 121: [参照付きランディングページを作成](https://x.com/Oluwaphilemon1/status/2079951300108697683) (by [@Oluwaphilemon1](https://x.com/Oluwaphilemon1))

**ランディングページ生成を試すときは、Kimi K3に強い視覚参照と反復的な指示を与えます。**

Oluwaphilemonは、画像参照と詳細なプロンプトを与えたランディングページ生成を説明しています。Kimiは最初に自転車を3Dモデル化しようとし、その後の指示でより良い結果に落ち着いたとされています。
...
<a id="case-128"></a>
### Case 128: [3D粒子システムをシミュレート](https://x.com/jadeferrara_/status/2079884161251262540) (by [@jadeferrara_](https://x.com/jadeferrara_))

**粒子シミュレーションのプロンプトで、Kimi K3の動きの質をクローズドモデルと比較します。**

Jade Ferraraは、Kimi K3とOpus 4.8に同じ3D粒子システムのプロンプトを与えたと報告しています。Kimiはより有機的な分布、滑らかな動き、低いAPIコストを示したとされています。
...
<a id="case-131"></a>
### Case 131: [深海解説パイプラインを作る](https://x.com/N01ennn/status/2080391368644657460) (by [@N01ennn](https://x.com/N01ennn))

**物語型の prompt を、シーン、テキスト、動き、解説を整理する視覚パイプラインに変換する。**

出典: N01ennn は Kimi K3 で深海テーマの解説コンテンツを作る流れを示している。投稿は、ナラティブ構造、ビジュアル、アニメーションを結び付ける具体的なデモとして有用。
...
<a id="case-133"></a>
### Case 133: [ランディングページをブラインドテストする](https://x.com/contraben/status/2080329043639947338) (by [@contraben](https://x.com/contraben))

**モデル名を伏せた landing page 比較で、Kimi K3 の frontend 出力を評価する。**

出典: contraben は複数モデルの landing page をブラインドテストとして公開している。このケースは視覚評価の方法を記録するもので、一般化された benchmark とはしない。
...
<a id="case-134"></a>
### Case 134: [機械的な 3D ディテールを試作する](https://x.com/0xDominiqq/status/2080316573210784230) (by [@0xDominiqq](https://x.com/0xDominiqq))

**Kimi K3 に、視覚ディテールとレンダリング制約を持つ小さな機械系 3D シーンを作らせる。**

出典: 0xDominiqq は Kimi K3 で作成した機械的または工業的なパーツを示している。有用な信号は、共有されたデモで確認できる精密な視覚プロトタイピングに限る。
...
<a id="case-135"></a>
### Case 135: [Expo モバイル UI を再現する](https://x.com/thebuggeddev/status/2080295524142587933) (by [@thebuggeddev](https://x.com/thebuggeddev))

**Kimi K3 を mobile Expo UI で試し、構造、コンポーネント、視覚的な仕上げを確認する。**

出典: thebuggeddev は Kimi K3 によるモバイル UI 再現を共有している。これは frontend 出力の評価であり、本番品質全体を保証するものではない。
...
<a id="case-139"></a>
### Case 139: [Web デザイン tutorial を追う](https://x.com/cyrilXBT/status/2080176332890345501) (by [@cyrilXBT](https://x.com/cyrilXBT))

**完整な tutorial を使い、Kimi K3 が layout、動き、視覚的な磨き込みをどう組むかを見る。**

出典: cyrilXBT は Kimi K3 でカスタムサイトを作る 13 分の tutorial を共有している。これは source-backed な学習 workflow であり、prompt gallery ではない。
...
<a id="case-119"></a>
### Case 119: [claude-code-routerでKimiを使う](https://x.com/sairahul1/status/2079393675885588855) (by [@sairahul1](https://x.com/sairahul1))

**不要なproxy modeを避けながら、claude-code-router経由で既存のClaude Code workflowにKimi K3を組み込む。**

Sai RahulはMoonshot支援のclaude-code-routerにおけるKimi K3対応を説明し、preset、API keyまたはsubscription import、native subscription routing、残高/利用量dashboard、failover、cache pricing、必要な場合以外proxy modeを切る注意点を挙げている。
...
<a id="case-125"></a>
### Case 125: [オープンなエージェントハーネスを実行](https://x.com/ShenSeanChen/status/2079914609222221976) (by [@ShenSeanChen](https://x.com/ShenSeanChen))

**共有エージェントハーネスを使い、ツール利用タスクでKimi K3と他の frontier モデルを比較します。**

Sean Chenは、エージェントループ、メモリ取得ゲート、ツール呼び出し、評価、トレース、コストボードを備えたオープンソースハーネスでKimi K3を競わせたと説明しています。
...
<a id="case-129"></a>
### Case 129: [Codex OAuthでKimiを実行](https://x.com/biscuitweb3/status/2079844959843197342) (by [@biscuitweb3](https://x.com/biscuitweb3))

**Kimi K3のコーディング作業で手動APIキー設定を避けたい場合、Codex OAuth経路を使います。**

biscuitweb3は、Kimi K3がOAuth経由でCodex内で動き、追加のAPIキー設定なしで選択して作業できると報告しています。添付スクリーンショットが統合の証拠です。
...
<a id="case-136"></a>
### Case 136: [production backend をストレステストする](https://x.com/hqmank/status/2080267878256005255) (by [@hqmank](https://x.com/hqmank))

**より現実的な backend フローで Kimi K3 の統合、コード推論、限界を調べる。**

出典: hqmank は production backend で Kimi K3 を試した内容を述べている。コード統合評価として扱い、独立した有料 runtime test は実行していない。
...
<a id="case-137"></a>
### Case 137: [opencodex 経由で Codex をルーティングする](https://x.com/cyrilXBT/status/2080252333829628163) (by [@cyrilXBT](https://x.com/cyrilXBT))

**Kimi K3 を Codex/opencodex ループに接続し、具体的な agent routing を試す。**

出典: cyrilXBT は Kimi K3 を使う opencodex 構成を共有している。ツール統合を記録しつつ、報告された設定とローカル検証の境界を保つ。
...
<a id="case-110"></a>
### Case 110: [AA-BriefcaseでKimi K3を評価](https://x.com/ArtificialAnlys/status/2079715807983210572) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))

**リーダーボードの主張を鵜呑みにする前に、AA-Briefcaseの結果でKimi K3の知識労働系エージェント能力を判断する。**

Artificial Analysisは、Kimi K3がAA-Briefcaseで1543 Eloを記録しClaude Fable 5に次ぐ2位だった一方、このベンチマークでは1タスク平均が約1時間で、Opus 4.8より高コストだったと報告している。
...
<a id="case-113"></a>
### Case 113: [フロントエンド順位の注意点を確認](https://x.com/RetroChainer/status/2079622227796885850) (by [@RetroChainer](https://x.com/RetroChainer))

**Kimi K3のleaderboard勝利と、より広いcodingやcost主張を切り分けるための制約メモとして使う。**

RetroChainerはFrontend Code Arena首位と公開価格を確認しつつ、順位はそのボード固有であること、投稿時点でweightsは未公開だったこと、作業単価の節約は1タスク由来であること、max reasoningが小さな作業でtokensを消費し得ることを指摘している。
...
<a id="case-114"></a>
### Case 114: [StackPerfの設計結果を比較](https://x.com/tamsi_besson/status/2079573266855834071) (by [@tamsi_besson](https://x.com/tamsi_besson))

**StackPerfでKimi K3とQwen 3.8の設計品質、速度、tool call挙動を比較する。**

Tamsi BessonはStackPerfの結果を共有し、Kimi K3がQwen 3.8 Max Previewを品質でわずかに上回り総時間も速い一方、Qwenはtool call失敗0件、Kimiは2件だったと述べている。
...
<a id="case-118"></a>
### Case 118: [Monument Valley風promptの費用比較](https://x.com/AiHubMix/status/2079507420083294557) (by [@AiHubMix](https://x.com/AiHubMix))

**単一のMonument Valley風promptで、Kimi K3と他モデルの視覚品質、実行時間、費用を比較する。**

AiHubMixはClaude Fable 5、Kimi K3、GPT-5.6 Solを1つのMonument Valley風promptで比較。Fableは最も一貫し、Kimiは52分30秒・$1.50で最も遅く、GPT-5.6 Solは最速最安だが空間ロジックは弱いと報告している。
...
<a id="case-120"></a>
### Case 120: [Surge指数でKimi K3を評価](https://x.com/echen/status/2080021575190110523) (by [@echen](https://x.com/echen))

**SurgeAIの分割結果を使い、Kimi K3の日常チャットでの強さと企業エージェントや科学での弱さを切り分けます。**

Eric Chenは、SurgeAIが日常チャットボット、企業エージェント、深い推論、先端科学を含む指数でKimi K3を評価したと報告しています。投稿では、日常チャットでは競争力がある一方、企業エージェントと科学ではFableやSolに遅れると述べています。
...
<a id="case-123"></a>
### Case 123: [PDEソルバーの誤りを評価](https://x.com/lanyon_ai/status/2079931884511887740) (by [@lanyon_ai](https://x.com/lanyon_ai))

**PDEベンチマークの失敗を使い、Kimi K3周辺で記号・数値タスクに検証が必要な箇所を判断します。**

Lanyon AIは、単純な線形PDEでKimi K3を含む frontier モデルと自社の神経記号アーキテクチャを比較しています。詳細なプロンプトでも数学的、アルゴリズム的、計算的な誤りが出たと報告しています。
...
<a id="case-124"></a>
### Case 124: [ClinicalBenchのEHRケースを確認](https://x.com/mkurman88/status/2079918374977413534) (by [@mkurman88](https://x.com/mkurman88))

**ClinicalBenchのEHRケースを使い、Kimi K3が成功する場面と診断推論が失敗する場面を確認します。**

Michael Kurmanは、仮想EHRサンドボックスの救急10ケースでKimi K3のClinicalBench v0.0.4結果を報告しています。Kimi K3は10件中7件を解き、6番目のケースで苦戦したとされています。
...
<a id="case-126"></a>
### Case 126: [税務計算を評価](https://x.com/michaelrbock/status/2079913117698666964) (by [@michaelrbock](https://x.com/michaelrbock))

**フロントエンドでの強さが税務計算にも移ると仮定せず、Kimi K3を個別にテストします。**

Michael Bockは、連邦および州の現実的な税申告50件でKimi K3をテストしたと報告しています。TaxCalcBenchではKimi K3が6%、GPT-5.6 Solが58%、Fable 5が4%だったとされています。
...
<a id="case-130"></a>
### Case 130: [複合 computer use ベンチマーク](https://x.com/YangFanYun/status/2080442432794325003) (by [@YangFanYun](https://x.com/YangFanYun))

**合成スコアだけでなく、複合的な computer use タスクで Kimi K3 を比較する。**

出典: YangFanYun は Kimi K3 を Gemini 3、GPT-5、GLM 4.6 と並べた複合 computer use benchmark として紹介している。長いタスクでの実用上の限界を見るための自己申告ベースの評価として扱う。
